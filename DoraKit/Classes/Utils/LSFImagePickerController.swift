...
@objc
public class LSFImagePickerController : UIImagePickerController {
    

    public static let album = LSFLocalized.string("title.photo.edit.album")
    public static let camera = LSFLocalized.string("str.Camera")
    public static let cancel = LSFLocalized.string("str.Cancel")
    ...
    ///强引用自己，防止被释放
    public var selfReference: LSFImagePickerController?
    ...
    public class func showActionsheet(controller:UIViewController,
        ...
        let imagePicker = LSFImagePickerController()
    ...
    public func showActionsheet(controller: UIViewController,
        ...
        lsf_alert(style: .actionSheet,
                  ...
                  sourceView: sourceView) {(title, index) in
                    if title == LSFImagePickerController.camera {
                        self.openCamera()
                    }
                        
                    else if title == LSFImagePickerController.album {
                        self.openAlbum()
                    }
                    else {
...
extension LSFImagePickerController : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if image == nil {
            ...
            LSFImagePickerController.fetchImage(for: asset) { (img, info) in
                image = img
        ...
        image = image?.lsf_fixOrientation() //相机拍出的照片方向有问题，需要修正下
...
public extension LSFImagePickerController {
...
public extension LSFImagePickerController {
    ...
    class func fetchImage(for asset: PHAsset?, complete: ((UIImage?, [AnyHashable: Any]?) -> Void)?) {
        ...
        manager.requestImageData(for: asset, options: options) { (data, UTI, imageOrientation, info) in
            ...
            if let image = UIImage(data: data)?.lsf_fixOrientation() {
