...
public class LSFMapUtil {
    ...
    public static func getInstallMapAppWithLocation(name: String,
        ...
        var appName = UIDevice.lsf_appName()
    ...
    public static func openMap(name: String,
        ...
        LSFMapUtil.openMap(name: name, location: location, backScheme: backScheme, sourceAppName: sourceAppName)
    }
    ...
    public static func openMap(name: String,
        ...
        let maps = LSFMapUtil.getInstallMapAppWithLocation(name: name, location: location, backScheme: backScheme)
       
        var mapNames = maps.map { (item) -> String in
            return item.name
        }
        mapNames.append("苹果地图")
        lsf_alert("打开本机地图导航", style: .actionSheet, actionTitles: mapNames, cancelTitle: "取消") { (title, index) in
            if title == "苹果地图" {
                LSFMapUtil.navAppleMap(location: location)
            }
            else {
                if maps.count > index {
                    let map = maps[index]
