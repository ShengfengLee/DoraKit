...
/*  使用实例
 func test() {
 var banners = [Any]() //数据源
 var bannerView = DoraCycleView.init(frame: CGRect.init(x: 0, y: 0, width: 375, height: 110))
...
/// 轮播图
public class DoraCycleView: UIView {
    ...
    var pageControl: DoraPageControl?
    var pageControlFrame: CGRect? {
        didSet {
            pageControl?.frame = self.pageControlFrame ?? CGRect(x: 5, y: 0, width: self.dora_width - 10, height: 10)
        }
    }
    ...
    private func configCellViews() {
        ...
        let currentTap = UITapGestureRecognizer.init(target: self, action: #selector(DoraCycleView.currentTap))
        currentView?.addGestureRecognizer(currentTap)
        
        forwordView?.isUserInteractionEnabled = true
        let tap2 = UITapGestureRecognizer.init(target: self, action: #selector(DoraCycleView.currentTap))
        forwordView?.addGestureRecognizer(tap2)
        
        nextView?.isUserInteractionEnabled = true
        let tap3 = UITapGestureRecognizer.init(target: self, action: #selector(DoraCycleView.currentTap))
    ...
    private func configPageControl() {
        ...
        pageControl?.frame = self.pageControlFrame ?? CGRect(x: 10, y: 0, width: self.dora_width - 20, height: 10)
    }
...
//        pageControl?.center = CGPoint(x: self.dora_width / 2.0, y: self.dora_height - 5)
    }
...
//MARK:- scrollView代理事件
extension DoraCycleView :UIScrollViewDelegate {
...
class DoraPageControl: UIPageControl {
