...
public extension String {
    static func dora_parse(_ anyObj:Any?) -> String? {
        var result:String?
        if anyObj is String {
    ...
    ///判断字符串是否有值
    static func dora_hasValue(_ string: String?) -> Bool {
    ...
    func dora_htmlAttributedString(font: UIFont) -> NSMutableAttributedString? {
    ...
    func dora_htmlParagraphText(font: UIFont,
                               firstLineHeadIndent: CGFloat = 0,
                               paragraphSpacing: CGFloat = 5,
                               lineSpacing: CGFloat = 5)  -> NSMutableAttributedString? {

        let attri = self.dora_htmlAttributedString(font: font)
    ...
    func dora_htmlFitImageSize(width: CGFloat) -> String {
        var html = ""
    ...
    func dora_labelHeight(width: CGFloat,
    ...
    func dora_labelWidth(height: CGFloat,
                         attributes: [NSAttributedString.Key: Any]?,
                         options: NSStringDrawingOptions = [.usesLineFragmentOrigin, .usesFontLeading]) -> CGFloat {
        let string = NSString(string: self)
...
//    func dora_md5(upper: Bool = false) -> String {
